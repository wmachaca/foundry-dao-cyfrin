...
function experimentalTernaries(node, path, print, { useTabs, tabWidth }) {
    ...
    const falseExpressionInSameLine = falseExpressionVariantKind === NonterminalKind.TupleExpression ||
...
function getOperandSingleExpression(operand) {
...
export class ConditionalExpression extends SlangNode {
    constructor(ast, options) {
        ...
        this.trueExpression = extractVariant(new Expression(ast.trueExpression, options));
        this.falseExpression = extractVariant(new Expression(ast.falseExpression, options));
        this.updateMetadata(this.operand, this.trueExpression, this.falseExpression);
        if (options.experimentalTernaries) {
            ...
            for (let operandSingleExpression = getOperandSingleExpression(this.operand); operandSingleExpression &&
                operandSingleExpression.kind !== NonterminalKind.ConditionalExpression; operandSingleExpression = getOperandSingleExpression(this.operand)) {
