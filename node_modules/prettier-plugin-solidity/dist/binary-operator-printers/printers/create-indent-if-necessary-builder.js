...
export const createIndentIfNecessaryBuilder = (shouldIndentMatchers) => (path) => (document) => {
    for (let i = 0, { node } = path;; i += 1) {
        const parentNode = path.getParentNode(i);
        if (notIndentParentTypes.includes(parentNode.type))
            return document;
...
//# sourceMappingURL=create-indent-if-necessary-builder.js.map