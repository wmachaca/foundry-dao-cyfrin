...
const experimentalTernaries = (node, path, print, options) => {
    ...
    const conditionAndTrueExpressionGroup = group([conditionDoc, trueExpressionDoc], { id: Symbol('Conditional.trueExpression') });
    // For the odd case of `tabWidth` of 1 or 0 we initiate `fillTab` as a single
    // space.
    let fillTab = ' ';
    if (!falseExpressionIsNested && // avoid processing if it's not needed
...
const traditionalTernaries = (path, print) => group([
...
export const Conditional = {
    print: ({ node, path, print, options }) => options.experimentalTernaries
