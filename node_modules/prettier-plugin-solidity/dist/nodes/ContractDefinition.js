...
import { printComments, printPreservingEmptyLines, printSeparatedItem, printSeparatedList } from '../common/printer-helpers.js';
...
const specifiers = (node, path, print) => {
    ...
    return printSeparatedList([group(document[0], { id: groupId }), document[1]], {
        firstSeparator: line,
...
const body = (node, path, options, print) => {
    ...
    return node.subNodes.length > 0 || (comments === null || comments === void 0 ? void 0 : comments.length)
        ? printSeparatedItem([printPreservingEmptyLines(path, 'subNodes', options, print), comments], { firstSeparator: hardline, grouped: false })
        : '';
};
