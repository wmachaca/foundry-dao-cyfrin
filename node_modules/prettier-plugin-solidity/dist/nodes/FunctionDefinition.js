...
import { printComments, printSeparatedItem, printSeparatedList } from '../common/printer-helpers.js';
...
const functionName = (node, options) => {
    ...
    const name = options.originalText.slice(options.locStart(node), options.locStart(node) + 8);
    return names[name];
};
const parameters = (parametersType, node, path, print, options) => {
    var _a, _b;
    if (((_a = node[parametersType]) === null || _a === void 0 ? void 0 : _a.length) > 0) {
        return printSeparatedList(path.map(print, parametersType), {
            grouped: false
        });
    }
    if (((_b = node.comments) === null || _b === void 0 ? void 0 : _b.length) > 0) {
        // we add a check to see if the comment is inside the parentheses
        const parameterComments = printComments(node, path, options, (comment) => getNextNonSpaceNonCommentCharacter(options.originalText, options.locEnd(comment)) === ')');
...
const visibility = (node) => node.visibility && node.visibility !== 'default'
...
const stateMutability = (node) => node.stateMutability ? [line, node.stateMutability] : '';
const modifiers = (node, path, print) => node.modifiers.length > 0
    ? [line, join(line, path.map(print, 'modifiers'))]
    : '';
const returnParameters = (node, path, print, options) => node.returnParameters
    ? [
        line,
        'returns (',
