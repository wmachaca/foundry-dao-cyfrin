...
const warnDeprecation = once(() => {
    printWarning(`The 'antlr' parser has been deprecated, please use 'slang' instead.`);
    return true;
});
function genericPrint(path, options, print) {
    ...
    if (hasNodeIgnoreComment(node)) {
        ignoreComments(path);
        return options.originalText.slice(options.locStart(node), options.locEnd(node) + 1);
    }
